...
export default function ChatInterface() {
  ...
  const handleSend = async (overrideText?: string) => {
    ...
    try {
      ...
      const message = err instanceof Error ? err.message : "Désolé, une erreur est survenue.";
      ...
      {dragOver && (
        <div className="absolute inset-0 z-50 bg-red-600/10 border-2 border-dashed border-red-400 rounded-lg flex items-center justify-center pointer-events-none">
          <div className="bg-white px-8 py-5 rounded-2xl shadow-lg text-center">
            ...
            <p className="text-red-700 font-semibold">Déposez votre image ici</p>
            <p className="text-gray-400 text-sm mt-1">Nous trouverons les prestataires correspondants</p>
          </div>
        </div>
      )}
                ...
                <div className="flex flex-col items-end gap-2">
                  {msg.imagePreview && (
                    <img
                      ...
                      alt="Uploaded"
                      className="max-h-32 rounded-xl border border-red-200 shadow-sm"
                    />
                  )}
                  {msg.content && (
